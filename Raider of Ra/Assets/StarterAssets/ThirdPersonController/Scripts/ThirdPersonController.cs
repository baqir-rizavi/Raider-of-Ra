...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [Tooltip("Camera motion Sensitivity")]
        ...
        public float DistanceFromAnckleToGround;
        ...
        private void Start()
        {
            ...
            _ikFoot = GetComponent<csHomebrewIK>();
        ...
        {
            if (animationEvent.animatorClipInfo.weight > 0.5f && _speed < 0.1f)
            {
                if (FootstepAudioClips.Length > 0)
                {
                    if (_ikFoot)
                        InstantiateFootPrint(animationEvent.intParameter);
        ...
        private void InstantiateFootPrint(int leftOrRight)
        {
            if (leftOrRight == 0)
            {
                var obj = Instantiate(leftFootPrint, new Vector3(0, 0.01f, 0) + _ikFoot.leftFootRayHitInfo.point, 
                    Quaternion.LookRotation(-transform.forward, _ikFoot.leftFootRayHitInfo.normal));
                Destroy(obj.gameObject, 4.5f);
            }
            else
            {
                var obj = Instantiate(rightFootPrint, new Vector3(0, 0.01f, 0) + _ikFoot.rightFootRayHitInfo.point, 
                    Quaternion.LookRotation(-transform.forward, _ikFoot.rightFootRayHitInfo.normal));
                Destroy(obj.gameObject, 4.5f);
            }
        }
